...
export const unpackMods = async () => {
    ...
    for (const file of packedMods) {
        ...
        if (
            ...
            throw new Error(
                `Couldn\'t load ${file.replace('.zip', '')}'s mod.json`,
            );
        ...
        if (dependencies) {
            for (const dep of dependencies) {
                ...
                if (!dependency.author)
                    throw new Error(
                        "Dependency format incorrect: can't find author",
                    );
                if (!dependency.name)
                    throw new Error(
                        "Dependency format incorrect: can't find name",
                    );
                if (!dependency.version)
                    throw new Error(
                        "Dependency format incorrect: can't find version",
                    );
