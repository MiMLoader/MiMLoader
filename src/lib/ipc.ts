...
class IPCServer {
        ...
        this.ws = {
            server: this.server.ws('/ws', {
                ...
                open: ws => {
                    ...
                    if (!this.wsConnections.has(ws.data.query.handler)) {
                        ...
                        console.warn(
                            `[IPC] ${ws.data.query.handler} already has an ipc connection, closing latest one.`,
                        );
                        ws.close();
                    }
                },
                message(ws, message) {
                    if (
                        message.channel === 'consoleLog' ||
                        message.channel === 'consoleWarn' ||
                        message.channel === 'consoleError'
                    ) {
                        console.log('[IPC][GAMELOG]', message.message);
                        if (message.channel !== 'consoleLog') {
                            console.warn(
                                `[IPC][GAMELOG][${message.channel}] ${message.message}`,
                            );
                        }
                    } else {
                ...
                close(ws) {
                    if (ws.data.query.handler === 'mimlAPI') {
                        console.warn(
                            '[IPC] Lost connection the game. Exiting Process.',
                        );
                        return process.exit(1);
                    }
                    console.log(
                        `[IPC] ${ws.data.query.handler}(${ws.id}) disconnected`,
                    );
                },
            }),
            send: (handler: string, channel: string, message: string) => {
                if (!this.wsConnections.get(handler)?.ws) {
                    console.warn(
                        handler,
                        "hasn't connected but an attempt to send data has been made",
                    );
                    return {
                        success: false,
                        message: "Game side ipc hasn't connected",
                    };
                }
                this.wsConnections
                    .get(handler)
    ...
    claim = (handler: string, process: Subprocess) => {
        // process.
    };
